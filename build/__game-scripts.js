...
LevelManager.prototype.initialize = function () {
    ...
    if (this.btn) {
        ...
        console.warn("Кнопка Btn2 не найдена");
    }
...
LevelManager.prototype.handleClick = function (e) {
    ...
    const i = this.camera.camera.screenToWorld(e.x, e.y, this.camera.camera.farClip);
    const l = this.app.systems.rigidbody.raycastFirst(t, i);

    if (!l || !l.entity) return;

    const r = l.entity.script && l.entity.script.cell ? l.entity :
        l.entity.parent && l.entity.parent.script && l.entity.parent.script.cell ? l.entity.parent : null;
    if (!r) return;
    ...
    if (n.clicked) {
        if (n.level === this.currentLevel) {
            ...
            if (this.firstClickedCell) {
                n.hide();
                this.firstClickedCell.hide();
                this.firstClickedCell = null;
                this.currentLevel++;

                if (this.nextCount >= 1 && this.currentLevel >= 5) {
                    this.currentLevel = 0;
                    this.nextCount++;
                }
                ...
                if (this.nextCount >= 2) {
                    Logger.getInstance().completedGame();
                    ProgressBar.instance.isRunning = false;
                }
            } else {
                this.firstClickedCell = n;
            }
        } else {
            if (this.firstClickedCell) {
                if (this.firstClickedCell.level === n.level) return;
                this.firstClickedCell.shake();
                this.firstClickedCell = null;
            }
            n.shake();
        }
